...
function showWeek() {
    ...
    var urlString = "http://localhost:8080/nexteer/oee/week/BEPS1?date="+curr_time;
    $.ajax({
        headers: {
...
$("#showMonth").bind("click",function (){
    ...
    $(document).ready(function () {
        ...
        console.log(MonthDate[d.getDate()-1]);
        {
            ...
            myMonthTitle.text = 'BEPS1可动率'+ Uyear+"-"+judgeTime(Umonth)+ '月视图';
            var IshaftOneoeeValueArr=[];
            var IshaftOneoeeTarArr=[];
            var urlString = "http://localhost:8080/nexteer/oee/month/BEPS1?date="+curr_time;
            $.ajax({
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
...
$("#selectMonthSub").bind("click",function (){
    ...
    var percent=50;
    {
        ...
        myMonthTitle.text = 'BEPS1可动率'+ data[0]+'-'+data[1]+ '月视图';
...
$("#showYear").bind("click",function () {
    ...
    {
        var curr_time=Uyear+"-"+judgeTime(Umonth)+"-"+judgeTime(Uday);
        myYearTitle.text = 'BEPS1可动率'+ Uyear + '年视图';
        var showoeeJson = new oeeInput(curr_time,"ISHAFT1");
        var IshaftOneoeeValueArr=[];
        var IshaftOneoeeTarArr=[];
        var urlString = "http://localhost:8080/nexteer/oee/year/BEPS1?date="+curr_time;
        $.ajax({
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
...
$("#selectYearSub").bind("click",function () {
    var IsOneoeeChart=echarts.init(document.getElementById('showIsOneWeekSheet'));
    {
        ...
        // var curr_time="2017-04-04";
        myYearTitle.text = 'BEPS1可动率'+ $("#selectYear").val() + '年视图';
        var showoeeJson = new oeeInput(curr_time,"ISHAFT1");
        var percent=99;
        var myData=[];
        console.log("开始传输数据");
