...
function showWeek() {
    ...
    var showhceJson = new hceInput(curr_time,"BEPS1");
    ...
    var urlString = "http://10.1.0.40:8080/nexteer/hce/week/BEPS1?date="+curr_time;
    $.ajax({
        headers: {
...
$("#showMonth").bind("click",function (){
    ...
    $(document).ready(function () {
        ...
        console.log(MonthDate[d.getDate()-1]);
        {
            ...
            myMonthTitle.text = 'BEPS1人员利用率'+ Uyear+"-"+judgeTime(Umonth)+ '月视图';
            var IshaftOnehceValueArr=[];
            var IshaftOnehceTarArr=[];
            var urlString = "http://10.1.0.40:8080/nexteer/hce/month/BEPS1?date="+curr_time;
            $.ajax({
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
...
$("#selectMonthSub").bind("click",function (){
    // window.location.assign("../../html/hceSec/BEPS1HceSec.html");
    var IsOnehceChart=echarts.init(document.getElementById('showIsOneWeekSheet'));
    var myData=[];
    var percent=50;
    {
        ...
        myMonthTitle.text = 'BEPS1人员利用率'+ data[0]+'-'+data[1]+ '月视图';
...
$("#showYear").bind("click",function () {
    ...
    {
        var curr_time=Uyear+"-"+judgeTime(Umonth)+"-"+judgeTime(Uday);
        myYearTitle.text = 'BEPS1人员利用率'+ Uyear + '年视图';
        var showhceJson = new hceInput(curr_time,"BEPS1");
        var IshaftOnehceValueArr=[];
        var IshaftOnehceTarArr=[];
        var urlString = "http://10.1.0.40:8080/nexteer/hce/year/BEPS1?date="+curr_time;
        $.ajax({
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
...
$("#selectYearSub").bind("click",function () {
    var IsOnehceChart=echarts.init(document.getElementById('showIsOneWeekSheet'));
    {
        ...
        // var curr_time="2017-04-04";
        myYearTitle.text = 'BEPS1人员利用率'+ $("#selectYear").val() + '年视图';
        var showhceJson = new hceInput(curr_time,"BEPS1");
        var percent=99;
        var myData=[];
        console.log("开始传输数据");
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (i, model) {
                ...
                IshaftOnehceValueArr[i]=model.BEPS1_value;
