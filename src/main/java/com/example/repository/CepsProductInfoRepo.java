...
public class CepsProductInfoRepo {
    ...
    @Autowired
    public CepsProductInfoRepo(@Qualifier("cepsJdbc") JdbcTemplate jdbc, @Qualifier("cell5Jdbc") JdbcTemplate cell5Jdbc) {
        this.jdbc = jdbc;
    ...
    public List<ProductInfo> getCell5ByPeriod(Date start, Date end){
        ...
        String startTime = timeFormat.format(start);
        String endTime = timeFormat.format(end);
        return cell5Jdbc.query("SELECT TimeStamp, NexteerModel FROM PackingList_CEPS WHERE ID IN (SELECT MIN(ID) " +
                "FROM PackingList_CEPS WHERE TimeStamp BETWEEN ? AND ? GROUP BY PartSerial) ORDER BY TimeStamp", new Object[]{startTime, endTime}, new AssemblyInfoMapper());
    }
    ...
    public List<Date> getCell5TopN(Date start, Date curr, int topN) {
        ...
        String startTime = timeFormat.format(start);
        String endTime = timeFormat.format(curr);
        String sql = "SELECT TOP (?) Timestamp FROM PackingList_CEPS WHERE ID IN (SELECT MIN(ID) " +
                "FROM PackingList_CEPS WHERE TimeStamp BETWEEN ? AND ? GROUP BY PartSerial) ORDER BY TimeStamp DESC";
